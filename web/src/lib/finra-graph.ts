...
async function loadBaselineGraph(profileName, { suppressRender = false }: { suppressRender?: boolean } = {}) {
	...
	graphData = payload && typeof payload === 'object' ? payload : { nodes: [], links: [], meta: {} };
	...
	sessionPersistenceMode = 'full';
	normalizeNodeLabelsInPlace(nodes);
	initialServerNodeIds = new Set(nodes.map((n) => n.id));
	initialServerLinkKeys = new Set(
		links.map((l) => {
			const s = l.source?.id ?? l.source;
			const t = l.target?.id ?? l.target;
 ...
 */
function coerceSearchHits(data) {
	if (!data) return [];
	...
	if (Array.isArray(data?.visibleNodes)) return data.visibleNodes;
	if (Array.isArray(data?.nodes)) return data.nodes;
	return [];
}

function normalizeSearchNode(hit) {
	...
	const kind =
		parsed?.kind ||
		(typeof parsed?.id === 'string' && parsed.id.includes(':individual:') ? 'individual'
		: typeof parsed?.id === 'string' && parsed.id.includes(':firm:') ? 'firm'
		: null);
	const crd = String(parsed?.basicInformation?.individualId ?? parsed?.individualId ?? src?.ind_source_id ?? src?.ind_crd ?? src?.crd ?? '').trim();
	const firmId = String(parsed?.firmId ?? src?.firm_id ?? src?.firmIdNumber ?? src?.firm_source_id ?? '').trim();
	const nodeId = String(parsed?.id || '').trim();
	const label = String(
		parsed?.label ||
			parsed?.title ||
			parsed?.name ||
			...
			[
				...
				.filter(Boolean)
				.join(' ') ||
			(kind === 'firm' ? `Firm ${firmId}`
			: crd ? `CRD ${crd}`
			: 'Search result'),
	).trim();

	return { src, parsed, kind, crd, firmId, nodeId, label };
}
...
async function loadGraph() {
	try {
		...
		// Keep the baseline graph visible for the default custom profile. The
		// empty-state branch here was suppressing the initial render and leaving
		// the page blank even when the graph backend had valid nodes to show.
		const shouldStartEmptyForCustomProfile = false;
...
function renderGraph(_data) {
	...
	const safeData = data && typeof data === 'object' ? data : { nodes: [], links: [], meta: {} };
	const nodes = Array.isArray(safeData.nodes) ? safeData.nodes.map((n) => ({ ...n })) : [];
	const nodeIdSet = new Set(nodes.map((n) => n.id));
	const allLinks = Array.isArray(safeData.links) ? safeData.links.map((l) => ({ ...l })) : [];
